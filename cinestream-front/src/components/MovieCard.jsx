...
export default function MovieCard({ movie, type = 'movie', isWatchlist = false, onRemove }) {
  const { user } = useAuth()
  ...
  const addToWatchlist = async (e) => {
    ...
    if (!user) return
    try {
      await api.post('/watchlist', { type, id: movie.id })
      setAdded(true)
    } catch {}
  }

  const poster = movie.poster || `https://via.placeholder.com/300x450/11131c/00daf3?text=${encodeURIComponent(movie.title)}`
      ...
      >
        <div className="aspect-[2/3] overflow-hidden relative">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 pointer-events-none">
            <h3 className="text-white font-bold text-sm md:text-base tracking-wide leading-tight mb-1 truncate">{movie.title}</h3>
            
            <div className="flex items-center justify-between transform transition-all duration-300 group-hover:-translate-y-1">
              <span className="text-white/60 text-[11px] font-medium tracking-wider">{movie.year}</span>
              <div className="flex items-center gap-1">
                <span className="material-symbols-outlined text-[#ffb950] text-[12px]" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                <span className="text-[#ffb950] text-[11px] font-bold">{movie.rating}</span>
              </div>
            </div>
